...
_EN = {
    ...
    "IP/自签名兼容": "IP/self-signed mode",
    "Host": "Host",
    "Host（可选）": "Host (optional)",
    "IP 直连时可填原域名，例如 llm.example.com": "For direct IP access, enter the original domain, for example llm.example.com",
    "留空会直接使用 Endpoint 的 IP；只有后端需要原域名路由时才填写。": "Leave empty to use the Endpoint IP directly; fill it only when the backend needs the original domain for routing.",
    "API Key": "API Key",
    "模型": "Model",
